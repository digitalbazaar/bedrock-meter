...
export function setInsertHandler({handler} = {}) {
  ...
  _checkHandlerNotSet({name: 'insert', handler: _insertHandler});
  _insertHandler = handler;
}

export function setRemoveHandler({handler} = {}) {
  assert.func(handler, 'handler');
  _checkHandlerNotSet({name: 'remove', handler: _removeHandler});
  _removeHandler = handler;
}

export function setUseHandler({handler} = {}) {
  assert.func(handler, 'handler');
  _checkHandlerNotSet({name: 'use', handler: _useHandler});
  _useHandler = handler;
}
