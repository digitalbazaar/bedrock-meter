...
bedrock.events.on('bedrock-mongodb.ready', async () => {
  ...
  await _insertMeters({meters: config.meter.initialMeters});
});
...
export async function update({meter, explain = false} = {}) {
  ...
  const collection = database.collections['meter-meter'];
  const query = {
    'meter.id': _decodeMeterId(meter.id),
...
export async function get({id, explain = false} = {}) {
  ...
  const collection = database.collections['meter-meter'];
  const query = {'meter.id': _decodeMeterId(id)};
...
function _decodeMeterId(meterId) {
  try {
    return Buffer.from(idDecoder.decode(meterId));
  } catch(e) {
    // wrap invalid meter ID in not found error
...
export async function _insertMeters({meters}) {
