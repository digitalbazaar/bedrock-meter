...
function _checkIsHandler({name, handler}) {
  // should be function like
  if(typeof handler !== 'function') {
    throw new BedrockError(
      'Meter handler must be a function.',
      'SetHandlerError', {
        name
      });
  }
...
bedrock.events.on('bedrock.start', async () => {
  console.log('METER BEDROCK.START');
  _checkHandlerSet({name: 'insert', handler: _insertHandler});
  _checkHandlerSet({name: 'remove', handler: _removeHandler});
  _checkHandlerSet({name: 'use', handler: _useHandler});
});
...
export function setHandler({name, handler} = {}) {
  switch(name) {
    case 'insert':
      _checkHandlerNotSet({name, handler: _insertHandler});
      _checkIsHandler({name, handler});
      _insertHandler = handler;
      break;
    case 'remove':
      _checkHandlerNotSet({name, handler: _removeHandler});
      _checkIsHandler({name, handler});
      _removeHandler = handler;
      break;
    case 'use':
      _checkHandlerNotSet({name, handler: _useHandler});
      _checkIsHandler({name, handler});
      _useHandler = handler;
      break;
    default:
      throw new BedrockError(
        'Meter handler name unknown.',
        'SetHandlerError', {
          name
        });
  }
